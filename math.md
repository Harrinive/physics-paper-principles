...
| **Quantifier audit** | Verify that quantifier order matches intent (∀∃ and ∃∀ are different conditions) and that each witness satisfies the declared domain. Require a nonempty or nontrivial witness only when the statement's intended meaning or explicit assumptions require one; empty or trivial witnesses may be mathematically valid. Determine whether zero is allowed in a proportionality statement from the stated convention or intended claim rather than excluding it universally. |
